...
contextBridge.exposeInMainWorld('lolCoach', {
  ...
  switchToBar:        ()     => ipcRenderer.send('switch-to-bar'),
  onCaptureState:     (cb)   => ipcRenderer.on('capture-session-state', () => cb()),
  sendCapturedState:  (state) => ipcRenderer.send('session-state-captured', state),
  onRestoreState:     (cb)   => ipcRenderer.on('restore-session-state', (_, state) => cb(state)),
  getDashboardWidgets:  ()  => ipcRenderer.invoke('get-dashboard-widgets'),
  saveDashboardWidgets: (w) => ipcRenderer.invoke('save-dashboard-widgets', w),
})
